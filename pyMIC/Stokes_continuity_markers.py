...
out_dict={'i':[],'j':[],'Lij':[],'Ri':[],'Si':[]};
#df=pd.DataFrame(columns=['i','j','Lij','Ri');
...
for i in range(0,L.shape[0]):
    for j in range(0,L.shape[1]):
        if L[i,j]!=0:
            out_dict['i'].append(i);
            out_dict['j'].append(j);
            out_dict['Lij'].append(L[i,j]);
            out_dict['Ri'].append(R[i]);
            out_dict['Si'].append(S[i]);

df=pd.DataFrame(out_dict);

df.to_excel('LijRiSi_from_Python.xls',index=None)
"""
"""
if L[2,1892]!=0:
    print('Checking');
    print('i={},j={}'.format(i,j));
    print('invx={}'.format(invx));
    print('invy={}'.format(invy));
