...
cdef int xsize,ysize,i,j,inp,invx,invy,marknum
cdef unsigned long int timesum,ntimestep
cdef int k,xnum,ynum,p0cell,gy,stepmax,mxnum,mynum,mm1,xm,ym,yn,xn
cdef double starttime,xstp,ystp,markmax,timemax,etaleft,etaright,etamin,dx,dy,kcont,kbond
cdef double mxstep,mystep,vxmax,vxm1,vym1
cdef numpy.ndarray x,y,xc,yc,gridx,gridy,gridcx,gridcy,MX,MY,MI,etas,etan
cdef numpy.ndarray wtetan,wtetas,MFLOW,MRHO,vx,vx1,vy,vy1,R,S,vxm,vym,rho,rho0,wtnodes
...
#for ntimestep in range(1,2):    
    ...
    L=lil_matrix((xnum*ynum*3,xnum*ynum*3),dtype=np.float64);
    ...
    plt.title(out);
    #plt.show();
    plt.savefig(f'out/pic{str(ntimestep)}.png',dpi=150)
    plt.close()
    ...
    pause(0.01) # Stop for 0.01 second
    """    
    #time.sleep(0.01);
    ...
    for mm1 in range(1,marknum):
        ...
        if (MX[mm1]>=gridx[0] and MX[mm1]<=gridx[xnum-1] and MY[mm1]>=gridy[0] and MY[mm1]<=gridy[ynum-1]): 
            ...
            vxm1=0;
            vxm1=vxm1+(1.0-dx)*(1.0-dy)*vx[yn,xn];
            vxm1=vxm1+(1.0-dx)*dy*vx[yn+1,xn];
            vxm1=vxm1+dx*(1.0-dy)*vx[yn,xn+1];
            vxm1=vxm1+dx*dy*vx[yn+1,xn+1];
            ...
            vym1=0;
            vym1=vym1+(1.0-dx)*(1.0-dy)*vy[yn,xn];
            vym1=vym1+(1.0-dx)*dy*vy[yn+1,xn];
            vym1=vym1+dx*(1.0-dy)*vy[yn,xn+1];
            vym1=vym1+dx*dy*vy[yn+1,xn+1];

            # Displacing Marker according to its velocity
            MX[mm1]=MX[mm1]+timestep*vxm1;
            MY[mm1]=MY[mm1]+timestep*vym1;
